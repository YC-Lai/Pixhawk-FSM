...
class Operation {
   private:
    ...
    /**
     * @brief Determines whether this operation is a operation we can be at for longer periods of
     * time. E.g. hold or land.
     */
    const bool steady;
   ...
   public:
     ...
     * @param steady Whether the operation is steady, it can be executed for longer periods of time
     ...
     *
     * @param should_tick               Called each tick, makes it possible to abort operations in
     * the midst of an execution.
     * @param should_halt_if_steady     Will halt at this operation if it's steady, is useful
     *                                  if we want to keep at a certain operation for some time,
     * e.g. #LandOperation or #HoldOperation.
     */
